...
def main(args):
    ...
    try:
        # Avoid hanging if run in non-interactive environment
        if sys.stdin.isatty():
            _ = input('')
        else:
            print("Non-interactive terminal detected. Auto-accepting TOS.")
    except EOFError:
        print("EOF encountered on input. Auto-accepting TOS.")
